...
#include <vision/GarbageRecognitionWithPrediction.h>
#include <vector>
#include <sstream>
#include <iostream>
#include <unistd.h>
...
typedef std::map<std::string, vision::IGarbageRecognition*> map_type;
map_type map;
...
void shutdownRobot(int a){
	robot->shutdown();
	exit(0);
}


template<typename T> vision::IGarbageRecognition * GarbageCleaner::createInstance(WorldInfo * wi) {
	return new T(wi);
}

GarbageCleaner::GarbageCleaner(WorldInfo * wi, robotapi::IRobot & robot, char * recognitionType = "GarbageRecognition" ) : myIRobot(robot)
// don't delete the following line as it's needed to preserve source code of this autogenerated element
// section -64--88-1-100--1230920c:125cb7ecd6f:-8000:0000000000000DB6 begin
{
	map["GarbageRecognition"] = createInstance<vision::GarbageRecognition>(wi);
	map["GarbageRecognitionWithPrediction"] = createInstance<vision::GarbageRecognitionWithPrediction>(wi);
	
	//(void) signal(SIGINT,shutdownRobot);
	(void) signal(SIGTERM,shutdownRobot);
	this->initializeSensors();
	this->initializeBehaviours(wi, recognitionType);
...
void GarbageCleaner::initializeBehaviours(WorldInfo * wi, char * garbageRecognitionType){

    behaviours::AbstractBehaviour * ab;
    
    gr = map[garbageRecognitionType];
	gr->setCamera(*camera);
