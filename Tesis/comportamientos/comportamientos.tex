...
\caption{Primera aproximaci\'on de ``Ir a la basura''. Inicialmente el objeto
...
\caption{Segunda aproximaci\'on de ``Ir a la basura''. Se traza un arco hacia
...
\caption{Posible inconveniente con la primera aproximac\'ion de
		``Ir a la basura''. La basura se encuentra en una de las
...
\caption{Conversi\'on de coordenadas de imagen de c\'amara.
	Dado un pixel ($y$,$x$), su nueva coordenada est\'a dada por
	($x-Crw/2$,$Crh-y$)}
...
\caption{Posibles estados iniciales de ``Entrar a la l\'inea''.
		En el caso (a) el robot debe girar hacia la derecha. En el
		caso (b), el robot debe girar hacia la izquierda. En el caso
		(c) el robot no debe girar}
...
\begin{figure}[htp]
\begin{center}
\includegraphics[scale=0.4]{comportamientos/floorSensorsLine4.png}
