...
moverse.
\paragraph{Implementaci\'on del comportamiento}
Para la implementaci\'on de este comportamiento nos basamos en un trabajo
anterior. El mismo consist\'io en ``conectar'' los sensores de distancia
(entrada) a los motores (salida) como se muestra en la figura \ref{fig:redN}. 
$W^s_i$ denota la conexi\'on entre el motor $s$ y el sensor de distancia $i$, y
posee un valor real entre -1 y 1. Si el sensor de distancia $i$ sensa un valor
de $d(i)$, entonces la velocidad otorgada al motor $s$ es:
\begin{equation*}
V(s) = \sum W^s_i d(i)
\end{equation*}
...
robot.
\\\indent
Como resultado obtuvimos los siguientes valores para los pesos de las
conexiones:
...
\caption{Conexionado entre los sensores de distancia y los motores}
\label{fig:redN}
\end{center}
\end{figure}
