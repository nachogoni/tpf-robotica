...
Usando combinaciones lineales de momentos de Hu de orden bajo se obtienen un descriptor 
...
objeto sin ser detectado y mantenido por el sistema de predicción. \\ 
...
sistema de predicción en \ref{tab:pred} \ref{tab:pred_2}.\\
\indent En los resultados arrojados en el cuadro \ref{tab:result} solo por el sistema de visión 
se observan problemas en la detección de colillas debido a los altos 
porcentajes de falsos negativos. Esto indica que el detector tiene 
problemas en distinguir a los objetos de su entorno ya que comete 
demasiados errores al detectar objetos que no existen. Esto concuerda 
con el hecho de que, en el caso de las colillas, el número de 
detecciones #D es siempre mayor que el #O.  Este no es el 
...
y platos con las colillas es entendible ya que las colillas son 
...
negativos, pero de forma mucho mas leve, haciendolo un trade-off 
favorable. En el caso de los vasos y platos no se observa ninguna 
mejora notoria. Para el caso de los vasos vemos un leve incremento 
...
reduce notablemente la cantidad de hits sin ver ninguna mejoras en la 
cantidad de detecciones falsas. Esto último puede ser atribuido a la 
baja cantidad de aciertos logrado por el predictor cuando el sistema 
de visión falló. 
\begin{table}[htb]
...
\begin{table}[htb]
  \begin{tabular}{|l | c | c | c | c | c | c | c | c |}
	...
	\end{tabular}
	\label{tab:pred}
	\caption{Performance del sistema con el sistema de predicción para 
...
\begin{table}[htb]
  \begin{tabular}{|l | c | c | c | c | c | c | c | c |}
	...
	\end{tabular}
	\label{tab:pred_2}
	\caption{Performance del sistema con el sistema de predicción con
