...
namespace behaviours {
    ...
    void GoToBase::action(){
		...
		double lspd = behaviours::BehavioursParameters::getParameter(GO_TO_BASE_BASE_SPD);
		double rspd = behaviours::BehavioursParameters::getParameter(GO_TO_BASE_BASE_SPD);
		
		// Left sensor is on line?
		if ( this->inLine((*this->fss).at(0)->getValue()) ){
			lspd = lspd * ( 1 - behaviours::BehavioursParameters::getParameter(GO_TO_BASE_SPD_FACTOR) );
			rspd = rspd * ( 1 + behaviours::BehavioursParameters::getParameter(GO_TO_BASE_SPD_FACTOR) );
		}
