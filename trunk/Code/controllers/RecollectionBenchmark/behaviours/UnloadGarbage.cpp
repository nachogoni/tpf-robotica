...
namespace behaviours {
    ...
    void UnloadGarbage::sense(){
   	...
   	void UnloadGarbage::action(){
		...
		for( int i = 0; i < behaviours::BehavioursParameters::getParameter(UNLOAD_WAIT_STEPS) ; i++ )
			this->robot->step(behaviours::BehavioursParameters::getParameter(UNLOAD_TIME_STEP));

		// Go forward a distance of behaviours::BehavioursParameters::getParameter(UNLOAD_E_PUCK_DIAMETER)
		this->wheels->setSpeed(behaviours::BehavioursParameters::getParameter(UNLOAD_BASE_SPD),behaviours::BehavioursParameters::getParameter(UNLOAD_BASE_SPD));

		goDistance(behaviours::BehavioursParameters::getParameter(UNLOAD_E_PUCK_DIAMETER));
		...
		this->trashbin->enable(behaviours::BehavioursParameters::getParameter(UNLOAD_TIME_STEP));
	...
	void UnloadGarbage::positionSelf(){
		...
		if ( ! ( currentAngle > 0 && currentAngle < PI ) )
   		...
   		this->wheels->setSpeed(behaviours::BehavioursParameters::getParameter(UNLOAD_FORWARD_SPD),behaviours::BehavioursParameters::getParameter(UNLOAD_FORWARD_SPD));
		this->goDistance(behaviours::BehavioursParameters::getParameter(UNLOAD_E_PUCK_DIAMETER)/2.0);
	...
	void UnloadGarbage::alignWithLine(){
		this->followLine(false,behaviours::BehavioursParameters::getParameter(UNLOAD_E_PUCK_DIAMETER));
		this->followLine(true,behaviours::BehavioursParameters::getParameter(UNLOAD_E_PUCK_DIAMETER));
    }
    ...
    void UnloadGarbage::followLineSpd(bool backwards){
		double lspd = behaviours::BehavioursParameters::getParameter(UNLOAD_BASE_SPD);
		double rspd = behaviours::BehavioursParameters::getParameter(UNLOAD_BASE_SPD);
		...
		if ( this->inLine((*this->fss).at(0)->getValue()) ){
			lspd = lspd * ( 1 - behaviours::BehavioursParameters::getParameter(UNLOAD_SPD_FACTOR) );
			rspd = rspd * ( 1 + behaviours::BehavioursParameters::getParameter(UNLOAD_SPD_FACTOR) );
		}
