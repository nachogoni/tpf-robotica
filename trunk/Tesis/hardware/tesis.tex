...
En esta secci\'on detallamos las caracter\'isticas f\'isicas del robot en
cuanto a dispositivos que lo componen, decisiones de dise\~no y armado del 
prototipo.\\
\indent Este apartado se divide de la siguiente forma: en la secci\'on \ref{h_ideas} exhibimos las principales ideas de 
implementaci\'on, en las secciones \ref{h_actuadores} y \ref{h_sensado} describimos los actuadores
y sensores utilizados, en la secci\'on \ref{h_controlador} mostramos los dispositivos de control
elegidos, en la secci\'on \ref{h_comm} detallamos el m\'etodo de comunicaci\'on dise\~nado
para la comunicaci\'on entre los controladores de cada perif\'erico, en la
secci\'on \ref{h_placas} exhibimos el dise\~no de las placas controladoras realizadas y en
la secci\'on \ref{h_prototipo} explicamos el armado del prototipo.
...
equilibrio, pero tambi\'en implicaba un sistema de actuaci\'on m\'as complejo desde el punto
de vista mec\'anico respecto al uso de ruedas.
Por otra parte, la coordinaci\'on de las patas y el equilibrio del cuerpo implicaba una
complejidad mec\'anica y de control, como as\'i tambi\'en un consumo mayor de energ\'ia
respecto a las otras posibilidades.
...
mejor posible, el per\'imetro del robot, otorgando un rango de detecci\'on lo suficientemente holgado
que permitiera una adecuada velocidad de respuesta al controlador principal para poder
evitar objetos desconocidos o identificados como obst\'aculos.
...
controlar la velocidad de las ruedas o para comunicar las distintas partes.
...
Con dos de estos motoreductores pod\'iamos garantizar la velocidad necesaria en cada una de las ruedas.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=1]{hardware/figures/MR2-FA.png}
...
La principal funci\'on del driver es proveer de la corriente y voltaje necesarios para el funcionamiento del motor, pero la configuraci\'on
...
Este integrado admite puentear ambas salidas, aumentando as\'i, la m\'axima corriente que puede circular por el motor.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.40]{hardware/figures/L298.png}
...
El consumo del motor lo medimos mediante los pines de sensado en el driver, conectados a masa por una resistencia y al m\'odulo de \emph{ADC} del
microcontrolador, que explicamos en la secci\'on \ref{h_controlador_micro_modulos}.
Conociendo el valor de la resistencia y el valor le\'ido por el m\'odulo de \emph{ADC}, pudemos determinar cu\'anta corriente circula por la
resistencia y por ende la corriente consumida por el motor.

Para controlar la velocidad del motor usamos uno de las dos salidas del encoder que trae.
Conectada como entrada para el m\'odulo de \emph{Timer} del microcontrolador, en configuraci\'on de contador, que explicamos m\'as en detalle en la
secci\'on \ref{h_controlador_micro_modulos}.
...
Cada $200$ms ($32$ interrupciones) tomamos la cantidad de cuentas del encoder y corregimos la velocidad de giro del motor ajustando el
...
Debido a que pensamos usar servos digitales y en una primera etapa del dise\~no se especific\'o el requerimiento
de $3$ servos, decidimos realizar la misma funci\'on pero por software.
...
\begin{table}[h]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Caracter\'isticas del sensor de distancia infrarrojo GP2D120.}
	\label{hT_gp2d120}
\end{table}

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.25]{hardware/figures/gp2d120_pulse.png}
...
\begin{figure}[h]
	\begin{minipage}[b]{0.5\linewidth}
		\centering
		\includegraphics[scale=0.25]{hardware/figures/tel-VxL.png}
		\caption{Voltaje seg\'un la distancia al objeto del tel\'emetro GP2D120.}
		\label{hF_gp2d120_distancia}
	\end{minipage}
	\begin{minipage}[b]{0.5\linewidth}
		\centering
		\includegraphics[scale=0.25]{hardware/figures/tel-VxX.png}
...
deb\'iamos tomar las muestras con el ADC.
Para filtrar el ruido de cada muestra individual, la distancia al objetivo es obtenida mediante el promedio
de una repetici\'on de mediciones.
...
\begin{table}[h]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Caracter\'isticas del sensor de ultrasonido SRF05.}
	\label{hT_srf05}
\end{table}

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.65]{hardware/figures/srf05_beam.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.25]{hardware/figures/srf05_pulse.png}
...
Cuando la interrupci\'on es activada significa que comienza al pulso con la distancia codificada en su ancho, por lo que tomamos una muestra del
timer y configuramos al pin para que genere ahora una interrupci\'on ante un flanco descendente.
Cuando la interrupci\'on es nuevamente activada, ser\'a porque termin\'o el pulso con la medici\'on, por lo que s\'olo debemos hacer la resta
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.25]{hardware/figures/cny70_dim.png}
...
\begin{figure}[h]
	\begin{minipage}[b]{0.45\linewidth}
		\centering
		\includegraphics[scale=0.25]{hardware/figures/cny70_diagrama.png}
	...
	\begin{minipage}[b]{0.45\linewidth}
		\centering
		\includegraphics[scale=0.25]{hardware/figures/cny70_IxD.png}
...
De igual forma que en los tel\'emetros utilizamos un transistor para conmutar la tensi\'on de alimentaci\'on en el sensor.
...
Los encoders son sensores que convierten una posici\'on lineal o angular en se\~nal el\'ectrica o tren de pulsos.
...
una de las dos salidas del encoder para conocer y controlar la velocidad a la que gira el rotor del motor.
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.35]{hardware/figures/bateria.png}
...
El rango de voltajes que analizamos es teniendo en cuenta la posibilidad de efectuar mediciones durante la carga de la bater\'ia
...
Adem\'as de los sensores que describimos, nos hace falta sensar switches que monitorean diversos estados del robot.
...
Utilizamos un cristal externo de 20MHz como clock.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.20]{hardware/figures/pic16f88.png}
...
Como mostramos en la figura \ref{hF_pic16f88_diagrama} cada pin se encuentra multiplexado con uno o m\'as unidades internas.

\paragraph{M\'odulos internos}
\label{h_controlador_micro_modulos}

Internamente el microcontrolador tiene una serie de unidades que proveen funciones extras y que utilizamos para lograr
...
\begin{figure}[b]
	\centering
	\includegraphics[scale=0.35]{hardware/figures/pic16f88_modulos.png}
	\caption{M\'odulos internos del microcontrolador PIC16F88.}
	\label{hF_pic16f88_modulos}
\end{figure}

Cuenta con $3$ timers, $2$ de $8$bits (\emph{TIMER0} y \emph{TIMER2}) y $1$ de $16$bits \newline (\emph{TIMER1}).
...
obtenemos una medida del paso del tiempo.
...
Se utiliza en conjunto con el \emph{TIMER2} fijando el prescaler y postscaler para determinar el ancho del
...
Este programador, a su vez, nos di\'o la posibilidad de depurar el c\'odigo en m\'as de una oportunidad.

En el cuadro \ref{hT_header_icd2} detallamos el conector de programaci\'on en circuito, con el cual establecemos la
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=.40]{hardware/figures/daisychain_diagram.png}
...
El protocolo de comunicaci\'on est\'a formado por paquetes que tienen un formato espec\'ifico y representan un pedido de
informaci\'on o comando que debe ser ejecutado en el destino.

El paquete consta de un encabezado com\'un con datos que identifican al emisor y receptor del paquete, el comando a enviar y
...
El campo \emph{LARGO} indica el tama\~no en bytes del paquete que viene a continuaci\'on.
Consta de 1 byte.
Esto es necesario porque el campo \emph{DATO} es de longitud variable.
...
Si el $ID$ de placa es 0x0F, el paquete es distribuido a todos los $ID$s del grupo indicado y si el valor es 0xFF, el paquete
es distribuido a todas las placas de todos los grupos.
...
y coloc\'andolo en el campo \emph{XOR}.
...
de inicializaci\'on, identificaci\'on de controladores, identificaci\'on de versi\'on o alg\'una otra necesidad
gen\'erica que se desarrolle en un futuro y deba ser incluida en el protocolo.
...
Existen grupos con sus comandos predefinidos.
...
\begin{table}
	\begin{center}
		...
		Valor & Comando & Detalle \\
		\hline
		0x40 & SET DIRECTION & Fija el sentido de giro del motor.
		Env\'ia el sentido de giro como par\'ametro. \\
		\hline
		0x41 & SET DC SPEED & Fija la velocidad del motor.
		...
		Env\'ia como par\'ametro el valor que debe tener el hist\'orico del encoder. \\
		\hline
		0x43 & GET ENCODER & Obtener las cuentas hist\'oricas del encoder.
		Responde con el valor hist\'orico del encoder. \\
		\hline
		0x44 & RESET ENCODER & Pone las cuentas hist\'oricas a cero. \\
		\hline
		0x45 & SET ENCODER TO STOP & Establece en cu\'antas cuentas debe detenerse el motor.
...
\begin{table}
	\begin{center}
		...
		\hline
		0x41 & SET ALL POSITIONS & Establece la posici\'on de cada uno de los servo motores.
		...
		\hline
		0x44 & SET SERVO SPEED & Fija la velocidad para el servo motor indicado.
		Env\'ia como par\'ametro el $ID$ del servo y la velocidad a setear. \\
		\hline
		0x45 & SET ALL SPEEDS & Establece la velocidad para cada servo motor.
...
\begin{table}[h]
	\begin{center}
		...
		\hline
		0x42 & SET FULL BIN VALUE & Establece el valor para determinar que el cesto est\'a lleno.
...
las posibilidades.
Adem\'as mantener distintos modos y protocolos dentro del proyecto generaba una carga de l\'ogica que pod\'iamos evitar.
...
los puertos y unidades del microcontrolador que elegimos a circuitos nuevos.
\'Esto nos sirvi\'o para realizar pruebas de concepto para la conexi\'on con los sensores, valores de componente pasivos
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=.25]{hardware/figures/rj11_db9.png}
...
que establece la configuraci\'on.
En la posici\'on \emph{LINK} la placa act\'ua como nodo intermedio y en la posici\'on \emph{LAST} como terminaci\'on.

Es de vital importancia colocar en forma correcta esta llave porque podemos dejar sin conexi\'on al resto de las placas o mismo
...
El header de programaci\'on \emph{P1} lo utilizamos para conectar la placa con el programador \emph{ICD2} y depurar
el c\'odigo como explicamos en en apartado \ref{h_controlador_micro_programacion}.
El switch \emph{S2} lo usamos para asociar los pines del microcontrolador con los canales de clock
y data del conector de programaci\'on (modo \emph{ICD2}) o con los pines \emph{RB6} y \emph{RB7 }del header
\emph{P2}.
...
con los conectores.
El m\'odulo de comunicaci\'on y conectores para conformar la cadena \emph{Daisy-Chain}
...
\begin{figure}
	\centering
	\includegraphics[scale=.32]{hardware/figures/gen_schemaMicro.png}
	\includegraphics[scale=.32]{hardware/figures/gen_schemaHeaders.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/gen_schemaComm1.png}
	\includegraphics[scale=.3]{hardware/figures/gen_schemaComm2.png}
	\includegraphics[scale=.3]{hardware/figures/gen_schemaComm3.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/gen_schemaFuente.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.2]{hardware/figures/gen_componentes.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.2]{hardware/figures/gen_top.png}
	\includegraphics[scale=.2]{hardware/figures/gen_bottom.png}
...
desde la velocidad del clock para los c\'alculos de tiempos hasta determinar qu\'e pines eran mapeados a las
funciones de RX y TX para la comunicaci\'on serial.

Para hacer pruebas sobre la comunicaci\'on RS232 o del protocolo, preparamos placas para las funciones que
atienden a los comandos espec\'ificos y funciones de control del protocolo comunes para todas las placas.
Tambi\'en generamos un modelo general del programa, con un loop principal y llamadas de inicializaci\'on.
...
El montaje de las placas se realiz\'o con componentes standard.
En una fase posterior a este proyecto final, el uso de montaje superficial nos permitir\'ia el dise\~no de
placas m\'as compactas.
...
Podemos enviar comandos para establecer estos valores y otros, como la cantidad de cuentas del encoder y a qu\'e
velocidad debe girar la rueda.
...
de 7 a 20 volts, con la posibilidad de alimentarla directamente con 5 volts a trav\'es de uno de los conectores.
...
El conector del motor comunica la placa con el motor de cont\'inua (en el cuadro \ref{hT_placa_dc_header_motor}
detallamos la posici\'on de los pines).

Las conexiones \emph{MOTOR\_B} y \emph{MOTOR\_A} son para la alimentaci\'on del motor.
Dependiendo del sentido de circulaci\'on de la corriente entre estos pines ser\'a el sentido de giro del motor.
...
motor.
En cambio las se\~nales \emph{Encoder\_A} o \emph{Encoder\_B} son pulsos en base a las vueltas del motor
antes de entrar en la caja.

El switch \emph{S2} lo utilizamos para asociar los pines del microcontrolador con el canal de datos del conector
de programaci\'on, que explicamos en la secci\'on \ref{h_controlador_micro_programacion}, permitiendo el modo
\emph{ICD2} o el modo de lectura de la se\~nal del encoder.
...
En la figura \ref{hF_placa_dc_schema} mostramos el esquem\'atico del microcontrolador y el conexionado el conector
de programaci\'on.
En la figura \ref{hF_placa_dc_schema2} detallamos el m\'odulo de comunicaci\'on y el conexionado con los conectores
entre placas.
En la figura \ref{hF_placa_dc_schema3} mostramos el esquem\'atico del driver y el header del motor.
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/dc_schemaMicro.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.28]{hardware/figures/dc_schemaComm1.png}
	\includegraphics[scale=.28]{hardware/figures/dc_schemaComm2.png}
	\includegraphics[scale=.28]{hardware/figures/dc_schemaComm3.png}
	\caption{Comunicaci\'on, switch de modo y conectores de entrada y salida.}
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/dc_schemaDriver.png}
	\includegraphics[scale=.3]{hardware/figures/dc_schemaMotorEncoder.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/dc_schemaADC.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/dc_schemaFuente.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/dc_componentes.png}
...
\begin{figure}
\centering
	\includegraphics[scale=.3]{hardware/figures/dc_top.png}
	\includegraphics[scale=.3]{hardware/figures/dc_bottom.png}
...
El c\'odigo m\'inimo para ser parte de la cadena de comunicaci\'on es similar en todas las placas que
armamos.
La personalizaci\'on del c\'odigo est\'a dentro de la funci\'on que interpreta el comando recibido y que realiza
las tareas internas necesarias para llevar a cabo dicho comando y depende \'integramente del protocolo.
...
Para mantener un hist\'orico de cuentas de encoder ocurridas nosotros usamos el conocimiento del sentido de giro
...
De forma similar, los tel\'emetros tienen un voltaje en salida dependiendo de la distancia con el objeto.
...
con la posibilidad de alimentarla directamente con 5 volts como explicamos en la secci\'on
\ref{h_placas_generica}.
...
Con la llave \emph{S2} cambiamos al modo programaci\'on vinculando el microcontrolador con el conector
de \emph{ICD2} o con la habilitaci\'on de los puertos de los sensores \emph{3} y \emph{4}.
Si esta llave est\'a en la posici\'on incorrecta nunca se excitar\'a el transistor y por ende no habr\'a
...
colocar un jumper en los pines de pull-up del puerto de sensor.

\paragraph{Esquem\'atico}
\label{h_placas_sensado_esquematico}

En la figura \ref{hF_placa_sense_schema} mostramos el esquem\'atico del microcontrolador y el conector
de programaci\'on.
...
\begin{figure}
	\centering
	\includegraphics[scale=.22]{hardware/figures/sense_schemaMicro.png}
	\caption{Microcontrolador y conector de programaci\'on.}
	\label{hF_placa_sense_schema}
\end{figure}

\begin{figure}
	\centering
	\includegraphics[scale=.28]{hardware/figures/sense_schemaComm1.png}
	\includegraphics[scale=.2]{hardware/figures/sense_schemaComm2.png}
	\includegraphics[scale=.28]{hardware/figures/sense_schemaComm3.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.28]{hardware/figures/sense_schemaSensor.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.22]{hardware/figures/sense_schemaFuente.png}
...
Esto generaba picos de consumo que, a pesar del filtrado, generaban un reset en el microcontrolador.
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/sense_componentes.png}
...
\begin{figure}
\centering
	\includegraphics[scale=.3]{hardware/figures/sense_top.png}
	\includegraphics[scale=.3]{hardware/figures/sense_bottom.png}
...
Los pines que excitan a la base de los transistores usan una l\'ogica inversa, es decir, un estado bajo habilita
...
servo motores, ya sea para el mecanismo de recolecci\'on de basura, inclinar el robot, reclinar el
...
A los puertos que no est\'an destinados al control de los servo motores les podemos dar varios usos.
En un principio los pensamos como generadores de interrupciones ante un cambio de estado y en el
...
Como mantuvimos la secci\'on de comunicaci\'on, para ser parte de la cadena s\'olo debemos incluir el
c\'odigo que implementa el protocolo e implementar las acciones necesarias dentro de la funci\'on que
...
De igual forma contemplamos el uso de switches en la misma.
...
Al igual que las otras placas utilizamos entre 7 y 20 volts para alimentarla, con la posibilidad
...
Usando la llave \emph{S3} establecemos la posici\'on de la placa dentro de la cadena, ya sea
...
\begin{figure}
	\centering
	\includegraphics[scale=.22]{hardware/figures/servo_schemaMicro.png}
	\caption{Microcontrolador y header de programaci\'on.}
...
\begin{figure}
	\centering
	\includegraphics[scale=.28]{hardware/figures/servo_schemaComm1.png}
	\includegraphics[scale=.2]{hardware/figures/servo_schemaComm2.png}
	\includegraphics[scale=.28]{hardware/figures/servo_schemaComm3.png}
	\caption{Comunicaci\'on, llaves de modo y conectores de entrada y salida.}
...
\begin{figure}
	\centering
	\includegraphics[scale=.35]{hardware/figures/servo_schemaPort.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/servo_schemaFuente.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.33]{hardware/figures/servo_componentes.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.3]{hardware/figures/servo_top.png}
	\includegraphics[scale=.3]{hardware/figures/servo_bottom.png}
...
Para capturar cambios de estado en los puertos de uso general s\'olo deber\'iamos configurar las
...
Posiblemente haya necesidades que no contemplamos en el dise\~no de esta placa aunque por no
...
que a su vez, nos daban el volumen interior para alojar las placas controladoras y bater\'ia.
...
El cableado con los sensores lo hicimos interior a trav\'es de agujeros en las paredes.
...
ninguna marca o corte para ajustar las ruedas, lo que dificult\'o el armado.
...
retirar del piso superior la netbook.
...
\begin{figure}
	\centering
	\includegraphics[scale=.45]{hardware/figures/desarme_e.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.45]{hardware/figures/desarme_f.png}
...
\begin{figure}
	\centering
	\includegraphics[scale=.45]{hardware/figures/desarme_i.png}
