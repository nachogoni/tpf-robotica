...
en las im\'agenes obtenidas. Probamos este algoritmo diseñandolo para la detecci\'on de 
...
n450 y 1 gb de memoria RAM como se detalla en ********************************************************************************************************.
...
procesamiento de im\'agenes y robots aut\'onomos.
	...
	%~ se recrea un ambiente hogareño compuesto por tres categor\'ias de 
	...
	\end{enumerate} Estas 
	caracter\'isticas se relacionan al color (1,2 y 3), tamaño (4), figura 
	...
	(2G -B -R) y la operaci\'on de threshold se segment\'o nuevamente la imagen 
	 ...
	 contiene un n\'umero mayor de hojas pequeñas que se solapan entre s\'i 
...
al ruido y a pequeñas alteraciones en dos formas iguales. Existen otros m\'etodos basados
...
traslaciones o escalas lo que los hace relativamente costosos. Ésto, sumado a la sensibilidad al ruido y a pequeños cambios en los bordes, hace que el m\'etodo 
...
dichos puntos y es convexa. Este m\'etodo obtiene dicha regi\'on y la compara con el contorno original obteniendo una serie de pequeñas
...
Sin embargo, tambi\'en establece que como solo usan parte de una figura, son m\'as sensibles al ruido y pequeñas variaciones.

\begin{figure}[tpb]
\begin{center}
  \includegraphics[scale=0.55]{vision/figures/shapess.png}
\end{center}
  \caption{Clasificaci\'on de las t\'ecnicas de descripci\'on y representaci\'on de figuras}
...
\end{center}
  \caption{Etapas del algoritmo de visi\'on}
  \label{fig:alg_steps}
\end{figure}
...
if H<0 then H=H+360
\end{verbatim}
\caption{\label{code:hsv}pseudo-c\'odigo de la conversi\'on RGB-HSV}
\end{figure}
...
tamaño es definido por el usuario. Siguiendo esta l\'ogica, se utilizan 
...
\end{center}
  \caption{\small Comparaci\'on de threshold adaptativo con el filtro de 
...
\end{center}
  \caption{\small Operaci\'on de dilataci\'on en im\'agenes binarias. Se reemplazan los pixeles no vac\'ios por una copia del elemento estructural con punto principal en esa posici\'on. La cruz indica la posici\'on del punto principal dentro del elemento estructural (ee). Imagen obtenida del curso de visi\'on artificial de la Universidad Polit\'ecnica de Madrid.}
...
\end{center}
  \caption{\small Ejemplo de segmentaci\'on en im\'agenes binarias. Las 1-componentes w1,w2 y w3 se encuentran rodeadas por la componente de fondo b1 y a su vez
...
\end{center}	
  \caption{\small Ejemplo de codificaci\'on de un contorno utilizando los c\'odigos de Freeman. }
...
\end{center}
  \caption{\small Matriz para la codificaci\'on de Freeman. El n\'umero indica la direcci\'on de desplazamiento del pr\'oximo punto respecto del
  actual (punto negro) }.
  \label{fig:freeman}
\end{figure}
...
\end{center}
  \caption{\small Aproximaci\'on de contornos mediante pol\'igonos. (a) Se observa que el contorno se encuentra acorde el borde del vaso, las
...
\end{center}
  \caption{\small Iteraciones del algoritmo de aproximaci\'on de pol\'igonos de Douglas-Peucker. (a) Imagen original. (b) Extracci\'on de contorno
...
\end{center}
  \caption{\small Ejemplo de descriptores de contorno. Se visualiza el \'area, per\'imetro, zona y circularidad del contorno 
...
regularidad de un objeto, la cantidad m\'axima de cuadros que puede 
...
performance de detecci\'on. En el caso de la variable $\delta$, el 
...
\indent La variable $\rho$, en cambio, establece otro tipo de balance. 
	Siempre que un objeto (de aparici\'on regular) deja de aparecer en la imagen (por 
	...
	(seg\'un el grado de regularidad de detecci\'on) y, en este caso, las equivoca a 
	todas ya que el objeto dejo de estar en el rango de visi\'on del 
	robot. El lado bueno es que cuando un objeto que ten\'ia un historial de 
	detecci\'on positivo es ocluido o sufre alguna alteraci\'on de tal 
	forma que el sistema de visi\'on deja de reconocerlo, el sistema de predicci\'on 
	...
	Es en esta \'ultima situaci\'on es donde se obtienen nuevas detecciones que el sistema de visi\'on 
	nunca podr\'ia obtener por s\'i solo.\\
	...
	valores de $\rho$, no muy pequeños, para afrontar el d\'eficit de 
	detecci\'on del sistema de visi\'on y valores de $\delta$, m\'as bien 
	pequeños, para no entorpecer la detecci\'on. Es importante remarcar 
...
\end{center}
  \caption{\small Se observa un vaso delimitado por un rect\'angulo 
...
\end{center}
  ...
  focalizaci\'on. El tamaño de la ventana es de tamaño proporcional al 
...
caracter\'isticas como tamaño y color que hacen que sea m\'as f\'acil 
reconocerlos y mas dif\'icil confundirlos en relaci\'on con estas 
\'ultimas, que poseen un color similar al del fondo y son de tamaño 
pequeño. En este caso, el sistema de predicci\'on no altera demasiado 
...
subyacente. En nuestro caso consideramos que tal y tal cosa es m\'as 
...
%~ el gran tamaño de los vasos y platos hace que se distingan f\'acilmente 
...
%~ porcentaje de falsos negativos es a\'un m\'as pequeña, resultando en un 
...
\begin{table}[htb]
  \begin{tabular}{|l | c | c | c | c | c | c |}
	...
	\hline
	\end{tabular}
	\caption{\label{tab:result} Performance del sistema sin el algoritmo de predicci\'on}
\end{table}
...
\begin{table}[htb]
  \begin{tabular}{|l | c | c | c | c | c | c | c | c |}
	...
	\hline
	\end{tabular}
	\caption{\label{tab:pred} Performance del sistema con el sistema de predicci\'on para 
...
\begin{table}[htb]
  \begin{tabular}{|l | c | c | c | c | c | c | c | c |}
	...
	\hline
	\end{tabular}
	\caption{\label{tab:pred_2} Performance del sistema con el sistema de predicci\'on con
...
\begin{table}[h]
  \begin{tabular}{|l | c | c | c | c | c|}
	...
	\hline
	\end{tabular}
	\caption{\label{tab:evo_vasos} Performance promedio de los vasos seg\'un configuraci\'on del 
...
\begin{table}[h]
  \begin{tabular}{|l | c | c | c | c | c|}
	...
	\hline
	\end{tabular}
	\caption{\label{tab:evo_platos} Performance promedio de los platos seg\'un configuraci\'on del 
	sistema y efectividad de las estimaciones del sistema de predicci\'on}
\end{table}
...
pequeñas perturbaciones que provoquen que un \'unico filtro rechaze un 
